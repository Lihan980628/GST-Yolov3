...
class ASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    ...
    def __init__(self, nc=80, anchors=(), ch=(), multiplier=0.5, rfb=False, inplace=True):  # detection layer
        ...
        self.register_buffer('anchors', torch.tensor(anchors).float().view(self.nl, self.na, 2))  # shape(nl,na,2)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                ...
                print('%10.1' 'list object cannot be interpreted as an integerf%10.0f%10.1fms %-40s' % (o, m.np, dt[-1], m.type))
    ...
    def info(self, verbose=False, img_size=416):  # print model information 图像分辨率
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR, ConvModule, Involution, InvolutionBottleneck, DeformConv2d, Inception, ASFFV5]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--cfg', type=str, default='yolov3-ghost-SPP-transformer-4detection.yaml', help='model.yaml')
    parser.add_argument('--cfg', type=str, default='yolov3-ghost.yaml', help='model.yaml')
